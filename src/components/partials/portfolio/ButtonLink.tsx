...
export default function ButtonLink({ link, icon, label, name }) {
  return (
    <a
      href={link || "#"}
      key={name || "no name"}
      className="group/button flex gap-2 items-center transition-all duration-800 text-sm lg:text-md w-fit h-fit p-2 rounded-sm text-slate-50 bg-blue-400 outline outline-inherit focus:outline-blue-400 focus:outline-offset-2 hover:rounded-full"
    >
      <FontAwesomeIcon
